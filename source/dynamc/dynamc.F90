...
#endif
#if KEY_SGLD==1 /*sgldprint*/
     IF (NSAVC > 0 .AND. PRNLEV > 0) THEN
        IF ((MOD(ISTEP, NSAVC) == 0) .AND. (QSGLD .OR. QSGMD)) THEN
...
#if KEY_SGLD==1 /*sgldprint*/
     if (nsavc > 0) then
        if((mod(istep, nsavc) == 0) .and. (qsgld .or. qsgmd)) &
           qdyncall = .false.
     end if
#endif /* (sgldprint)*/
