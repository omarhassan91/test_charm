...
335           FORMAT(' <AUTGEN>: ',I5, &
  ...
  IF(LTHETA) THEN
     DO I=1,NBOND
        IBT=IB(I)
        JBT=JB(I)
        ! BIOVIA Code Start : Fix for TIP3
        ! Skip H-H bond in TIP3 water
        IF (ATYPE(IBT)(1:1) == 'H' .AND. ATYPE(JBT)(1:1) == 'H') CYCLE
        ! BIOVIA Code End
        DO J=1,NATBON(IBT)
           ! BIOVIA Code Start : Fix for TIP3
           IF (ATYPE(IBT)(1:1) == 'H' .AND. ATYPE(JBT)(1:3) == 'OH2') CYCLE
           ! BIOVIA Code End
           IJ=IATBON(J,IBT)
           IF(IJ.GT.0) THEN
              IJA=JB(IJ)
        ...
        DO J=1,NATBON(JBT)
           ! BIOVIA Code Start : Fix for TIP3
           IF (ATYPE(JBT)(1:1) == 'H' .AND. ATYPE(IBT)(1:3) == 'OH2') CYCLE
           ! BIOVIA Code End
           IJ=IATBON(J,JBT)
           IF(IJ.GT.0) THEN
              IJA=JB(IJ)
  ...
  DO I=1,NBOND
     ...
     ! BIOVIA Code Start : Fix for TIP3
     ! Skip H-H bond in TIP3 water
     IF (ATYPE(IBT)(1:1) == 'H' .OR. ATYPE(JBT)(1:1) == 'H') CYCLE
     ! BIOVIA Code End
     DO J=1,NATBON(IBT)
        IJ=IATBON(J,IBT)
        IF(ABS(IJ).NE.I) THEN
